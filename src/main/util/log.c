...
void log_print(log_level_t const level, char const* fmt, ...)
{
  ...
  va_list args;
  va_start(args, fmt);
  log_printer_func(output, level, fmt, args);
  va_end(args);
}

void log_default_printer(FILE* output, log_level_t const level, char const* fmt, va_list args)
{
